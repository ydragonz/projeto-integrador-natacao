...
if($_SESSION['logado']) {
    ...
    $sql = "SELECT * FROM telefones t INNER JOIN atletas a ON t.id_atleta=a.id_atleta ORDER BY id_telefone";
    $res = $conn->query($sql);
    ?>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        if($_SESSION['sts_usuario']) {
            ?>
            <a href="main.php?p=telefones/new.php" type="button" class="btn btn-success">Cadastrar</a>
            <?php
        }
        ?>
    </div>
    ...
    if(isset($_GET['del'])) {
        $id = $_GET['del'];
        $conn->query("DELETE FROM telefones WHERE id_telefone=$id");
        ?>
        <div class="alert alert-success" role="alert">
            ...
            Telefone excluído com sucesso!</h2>
    ...
    if($res->num_rows>0){
        ...
        <div class="table-responsive">
            <table class="table table-striped table-sm" id="tabela_telefones">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    while($row = $res->fetch_assoc()){
                        ...
                        echo "<tr>
                            ...
                            <td>".$tel."</td>
                            <td>".$row['ddd_telefone']."</td>
                            <td>".$row['num_telefone']."</td>";
                        if($_SESSION['sts_usuario']) {
                            echo "<td class='edit_test'><a href='main.php?p=telefones/edit.php&id=".$row['id_telefone']."' class='btn btn-primary btn-sm'>Editar</a>
                                <a href='main.php?p=telefones/index.php&del=".$row['id_telefone']."' class='btn btn-danger btn-sm'>Excluir</a></td></tr>";
                        }
                        else {
                            echo "<td></td></tr>";
                        }
