...
if($_SESSION['logado']) {
    ...
    $sql = "SELECT * FROM alergias a INNER JOIN atletas b ON a.id_atleta=b.id_atleta ORDER BY id_alergia";
    $res = $conn->query($sql);
    ?>

    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Alergias</h1>
        <?php
        if($_SESSION['sts_usuario']) {
            ?>
            <a href="main.php?p=alergias/new.php" type="button" class="btn btn-success">Cadastrar</a>
            <?php
        }
        ?>
    </div>
    ...
    if(isset($_GET['del'])) {
        $id = $_GET['del'];
        $conn->query("DELETE FROM alergias WHERE id_alergia=$id");
        ?>
        <div class="alert alert-success" role="alert">
            ...
            Alergia excluída com sucesso!</h2>
    ...
    if($res->num_rows>0){
        ...
        <div class="table-responsive">
            <table class="table table-striped table-sm" id="tabela_alergias">
                <thead>
                    <tr>
